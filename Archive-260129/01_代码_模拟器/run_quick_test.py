...
class Player:
    ...
    farming_count: int = 0
    transfer_targets: Set[int] = field(default_factory=set)
    kindness_targets: Set[int] = field(default_factory=set)
...
class QuickSimulator:
    ...
    def execute_action(self, player, action, all_players):
        ...
        if action == "donate" and player.wealth >= 3:
            ...
            target.merit += int(1 * m)  # 文化道时目标也翻倍
...
def run_test(num_players, games=5000):
    print(f"\n{'='*50}")
    print(f"测试 {num_players} 人模式 ({games} 局)")
    print(f"{'='*50}")
    
    sim = QuickSimulator(num_players)
    ...
    for path in list(Path)[:num_players]:
        ...
        print(f"  {name}: {rate}% (偏差: {deviation:+.1f}%) {status}")
    
    balance_score = sum(abs(r - target_rate) for r in rates.values()) / len(rates)
    status = "✓ 通过" if balance_score < 5 else "✗ 需调整"
    print(f"\n  平衡分数: {balance_score:.2f} {status}")
    
    return rates, balance_score

if __name__ == "__main__":
    print("="*60)
    print("效率加成改为翻倍后的平衡性验证")
    print("布施道: 1.75 → 2.0 (翻倍)")
    print("文化道: 1.5 → 2.0 (翻倍)")
    print("="*60)
