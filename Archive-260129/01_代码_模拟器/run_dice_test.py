# -*- coding: utf-8 -*-
"""
Simplified dice test runner - avoid encoding issues
"""
...
class Player:
    ...
    def __post_init__(self):
        bonus = PATH_STARTUP.get(self.path, {})
...
class GameSimulator:
    ...
    def create_game(self, paths=None, strategies=None):
        ...
        players = [Player(i, paths[i], strategies[i], self.num_players) 
    ...
    def decide_action(self, player, game, others):
        ...
        if player.strategy == AIStrategy.RANDOM:
            ...
            if round_num <= 3:
                ...
                return self.decide_action(Player(player.player_id, player.path, 
                                                  AIStrategy.BALANCED, player.num_players), 
                                          game, others)
            else:
                if "build_temple_land" in actions:
                    return "build_temple_land"
    ...
    def run_game(self, paths=None, strategies=None):
        ...
        for p in game["players"]:
            ...
            path_complete, completion_type, path_bonus = p.check_path_completion(self.num_players)
            results.append({
                "path": p.path.value,
                "score": round(score, 1),
                "dice_wealth": p.dice_wealth_bonus,
                "dice_influence": p.dice_influence_bonus,
            })
...
def run_test(num_players, games, use_dice, dice_config=None):
    sim = GameSimulator(num_players, use_dice, dice_config)
    ...
    dice_stats = dict(sim.dice_stats) if use_dice else {}
...
def main():
    print("=" * 60)
    print("Production Dice System A/B Test")
    print("=" * 60)
    ...
    for num_players in [4, 5, 6]:
        ...
        # Control: no dice
        print(f"  Testing control (no dice)...")
        start = time.time()
        control = run_test(num_players, 10000, False)
        print(f"  Done in {time.time()-start:.1f}s")
        
        # Experiment: with dice
        print(f"  Testing experiment (with dice)...")
        start = time.time()
        experiment = run_test(num_players, 10000, True)
        print(f"  Done in {time.time()-start:.1f}s")
        
        results[f"{num_players}p"] = {
            "control": control,
        ...
        for path in Path:
            name = path.value
            c_rate = control["win_rates"].get(name, 0)
            e_rate = experiment["win_rates"].get(name, 0)
            change = e_rate - c_rate
            status = "OK" if abs(e_rate - target) < 8 else "BIAS"
            print(f"    {name}: {c_rate:.1f}% -> {e_rate:.1f}% ({change:+.1f}%) [{status}]")
        
        print(f"\n  Score Std Comparison:")
        for path in Path:
            name = path.value
            c_std = control["score_stats"].get(name, {}).get("std", 0)
            e_std = experiment["score_stats"].get(name, {}).get("std", 0)
            change = e_std - c_std
            print(f"    {name}: {c_std:.2f} -> {e_std:.2f} ({change:+.2f})")
        
        if experiment["dice_stats"]:
            print(f"\n  Dice Distribution:")
            total = sum(experiment["dice_stats"].values())
            for result, count in experiment["dice_stats"].items():
                print(f"    {result}: {count} ({count/total*100:.1f}%)")
    
    # Save results
    with open("dice_ab_test_results.json", "w") as f:
        json.dump(results, f, indent=2)
    
    print("\n" + "=" * 60)
    print("Results saved to dice_ab_test_results.json")
    print("=" * 60)
