...
class ComprehensiveTester:
    def __init__(self, num_players: int = 4):
        ...
        self.underdog_wins = 0  # 弱者翻盘
        self.underdog_ascensions = 0  # 弱者转职
        self.total_games = 0
        
        # 策略×道路组合统计
        self.path_strategy_wins = defaultdict(lambda: defaultdict(int))
        self.path_strategy_games = defaultdict(lambda: defaultdict(int))
    
    def run_symmetric_test(self, num_games: int):
        """对称测试：均匀覆盖所有道路组合"""
        ...
        actual_games = games_per_combo * len(path_combos)
        
        print(f"  [Symmetric Test] {len(path_combos)} path combos x {games_per_combo} games = {actual_games}")
    ...
    def run_strategy_test(self, num_games: int):
        """策略测试：均匀覆盖所有策略组合"""
        all_paths = list(Path)[:self.num_players]
        
        strategy_combos = []
        for _ in range(min(100, num_games)):
            combo = tuple([random.choice(ALL_STRATEGIES) for _ in range(self.num_players)])
            strategy_combos.append(combo)
        
        games_per_combo = max(1, num_games // len(strategy_combos))
        actual_games = games_per_combo * len(strategy_combos)
        
        print(f"  [Strategy Test] {len(strategy_combos)} strategy combos x {games_per_combo} games = {actual_games}")
        
        for strategies in strategy_combos:
            for _ in range(games_per_combo):
                paths = random.sample(all_paths, self.num_players)
                self._run_and_record(paths, list(strategies))
    
    def run_mixed_test(self, num_games: int):
        """混合测试：随机道路和策略"""
        all_paths = list(Path)[:self.num_players]
        
        print(f"  [Mixed Test] {num_games} random games")
        
        for _ in range(num_games):
            paths = random.sample(all_paths, self.num_players)
            strategies = [random.choice(ALL_STRATEGIES) for _ in range(self.num_players)]
            self._run_and_record(paths, strategies)
    ...
    def _run_and_record(self, paths: List[Path], strategies: List[AIStrategy]):
        ...
        for r in result["results"]:
            ...
            self.path_strategy_games[path][strategy] += 1
    ...
    def run_comprehensive_test(self, num_games: int):
        """运行全面测试"""
        print(f"\n{'='*70}")
        print(f"Comprehensive Test: {self.num_players} players, {num_games} games total")
        print(f"{'='*70}")
        
        # 分配测试比例
        symmetric_games = num_games // 3
        strategy_games = num_games // 3
        mixed_games = num_games - symmetric_games - strategy_games
        
        start = time.time()
        
        self.run_symmetric_test(symmetric_games)
        self.run_strategy_test(strategy_games)
        self.run_mixed_test(mixed_games)
        
        elapsed = time.time() - start
        print(f"\n  Total time: {elapsed:.2f}s ({self.total_games / elapsed:.1f} games/s)")
    ...
    def print_report(self):
        ...
        print(f"\n{'='*70}")
        print(f"                    TEST REPORT - {self.num_players} PLAYERS")
        print(f"{'='*70}")
        print(f"Total games: {report['total_games']}")
        print(f"Target win rate: {target_rate:.1f}%")
        print(f"Balance score: {report['balance_score']:.2f} (lower is better)")
...
def main():
    print("=" * 70)
    print("       Path to Salvation v7.0 - Comprehensive Balance Test")
    print("=" * 70)
    ...
    for num_players in [4, 5, 6]:
        ...
        tester.run_comprehensive_test(10000)
        report = tester.print_report()
        all_reports[f"{num_players}p"] = report
    
    # 保存结果
    with open("v7_comprehensive_test_results.json", "w", encoding="utf-8") as f:
        json.dump(all_reports, f, ensure_ascii=False, indent=2)
    
    print(f"\n{'='*70}")
    print("Results saved to v7_comprehensive_test_results.json")
    print(f"{'='*70}")
