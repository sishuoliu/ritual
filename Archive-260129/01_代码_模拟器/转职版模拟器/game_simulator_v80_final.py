...
救赎之路 v8.0 - 精细平衡版
基于v7.9结果的调整：
1. 布施道被削弱过度（6-11%）→ 略微增强
2. 渡化道太强（28-42%）→ 削弱
3. 勤劳道5-6人太强（22-33%）→ 削弱
"""
...
class Player:
    ...
    def __post_init__(self):
        ...
        if self.path == Path.NIRVANA:
            ...
            # 布施道：转职型，略微增强（v7.9削弱过度）
            self.wealth += 4  # 从+3增回+4
            ...
            # 渡化道：转职型，削弱
            self.merit += 3  # 从+4降到+3
            self.influence += 1  # 从+2降到+1
    ...
    def update_path_completion(self):
        if self.path == Path.NIRVANA:
            ...
            # 渡化道小成条件提高
            if self.transfer_count >= 2 or len(self.kindness_targets) >= 2:  # 从1/1提高到2/2
                self.small_path_completed = True
    ...
    def check_early_victory(self, players: List['Player']) -> bool:
        ...
        if self.ascension_class == AscensionClass.ARHAT:
            ...
            # 布施道：略微降低难度
            return (self.total_donated >= 42 and  # 从50降到42
                    self.wealth <= 3 and
                    self.merit >= 10)
            ...
            return (len(all_helped) >= num_players - 1 and 
                    self.merit >= 15 and  # 从12提高到15
                    self.transfer_count >= 5)  # 从4提高到5
    ...
    def get_ascension_bonus_multiplier(self, action: str) -> float:
        ...
        bonuses = {
            ...
            (AscensionClass.MAHADANAPATI, "donate"): 1.4,  # 从1.3增到1.4
            (AscensionClass.TEMPLE_MASTER, "donate_land"): 1.5,
            (AscensionClass.TEMPLE_MASTER, "farm"): 1.3,
            (AscensionClass.MANJUSRI, "kindness"): 1.5,
            (AscensionClass.WANDERING_MONK, "farm"): 1.2,  # 从1.3降到1.2
            (AscensionClass.WANDERING_MONK, "kindness"): 1.2,  # 从1.3降到1.2
            (AscensionClass.BODHISATTVA, "transfer"): 1.5,  # 从2.0降到1.5
        }
    ...
    def check_path_completion(self) -> Tuple[bool, str, int]:
        ...
        conditions = {
            ...
            Path.CHARITY: (
                self.total_donated >= 14 and self.influence >= 3,
                self.total_donated >= 8,
                "布施道"
            ),
            Path.TEMPLE: (
            ...
            Path.SALVATION: (
                self.transfer_count >= 4 and len(self.transfer_targets | self.kindness_targets) >= 3,  # 提高条件
                self.transfer_count >= 2 or len(self.kindness_targets) >= 2,  # 提高条件
                "渡化道"
            ),
        }
...
class GameSimulator:
    ...
    def _finish_game(self, game: Dict, early_winner: Optional[Player], victory_type: VictoryType) -> Dict:
        results = []
        for p in game["players"]:
            results.append({"path": p.path.value, "strategy": p.strategy.value, "ascended": p.has_ascended, "initial_rank": p.initial_rank, "mid_game_rank": p.mid_game_rank, "score": round(p.get_final_score(), 1)})
...
def main():
    ...
    with open("v80_results.json", "w", encoding="utf-8") as f:
