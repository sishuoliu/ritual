"""
《功德轮回：众生百态》v1.5 僧侣数值调优版

调整：
1. 托钵收益提升：+2财富+2福+2慧（原+1财+1福+2慧）
2. 讲法收益提升：给他人+2福+1慧，自己+2福+1慧（原自己只+1福）
3. 法事惩罚加重：+4财富-2福-2慧（原+3财-1福-1慧）
4. 供养收益提升：双方各+2福（原各+1福）
5. 农夫劳作削弱：不再有额外福
6. 调整其他角色能力
"""
...
# 角色初始值（调整）
ROLE_INIT = {
    RoleType.FARMER: {"wealth": 5, "fu": 1, "hui": 1},   # 无特殊优势
    RoleType.MERCHANT: {"wealth": 7, "fu": 0, "hui": 1}, # 高财富
    RoleType.OFFICIAL: {"wealth": 5, "fu": 1, "hui": 2}, # 慧优势
    RoleType.MONK: {"wealth": 1, "fu": 2, "hui": 4},     # 低财富，高福慧
}
    ...
    def _choose_action(self, player: Player, strategy: Strategy, player_idx: int) -> ActionType:
        ...
        if is_monk:
            if strategy == Strategy.SELFISH:
                return ActionType.CEREMONY  # 贪财做法事
            
            elif strategy == Strategy.ALTRUISTIC:
                if calamity_urgent and player.wealth >= 2:
                    return ActionType.PROTECT
                if player.wealth >= 3 and affordable_beings:
                    return ActionType.SAVE
                return ActionType.TEACH  # 优先讲法
                ...
                if player.fu < player.hui:
                    return ActionType.TEACH  # 福不够，讲法
                if player.hui < player.fu:
                    return ActionType.ALMS  # 慧不够，托钵
        ...
        if strategy == Strategy.SELFISH:
            ...
            if calamity_urgent and player.wealth >= 2:
                return ActionType.PROTECT
            if has_monk and player.wealth >= 4:
                monk = self.state.players[monk_idx]
                if monk.wealth < 2 and random.random() > 0.4:
            ...
            if player.wealth < 4:
                return ActionType.LABOR
            if has_monk and player.wealth >= 5:
                monk = self.state.players[monk_idx]
                if monk.wealth < 2 and random.random() > 0.5:
                    return ActionType.SUPPORT_MONK
            ...
            if urgent_beings:
                ...
                if affordable_urgent:
                    return ActionType.SAVE
            if has_monk and player.wealth >= 5 and random.random() > 0.6:
                monk = self.state.players[monk_idx]
                if monk.wealth < 3:
                    return ActionType.SUPPORT_MONK
...
class MonteCarloTester:
    ...
    def run_full_test(self):
        print("=" * 80)
        print("《功德轮回》v1.5 僧侣数值调优版")
        print(f"模拟次数: {self.num_simulations}局/配置")
        print("=" * 80)
        
        print("\n【僧侣机制（调优后）】")
        print("• 托钵（清修）：+2财富+2福+2慧")
        print("• 法事（应酬）：+4财富-2福-2慧")
        print("• 讲法：给他人+2福+1慧，自己+2福+1慧")
        print("• 供养：他人-2财富+2福，僧侣+2财富+1福")
        ...
        for name, strategies in configs:
            ...
            role_data = defaultdict(lambda: {"wins": 0, "fu": [], "hui": [], "wealth": [], "scores": []})
        ...
        print(f"\n贪财僧侣（做法事）: 福={statistics.mean(selfish_monk['fu']):.1f}, 慧={statistics.mean(selfish_monk['hui']):.1f}")
        print(f"清修僧侣（托钵+讲法）: 福={statistics.mean(smart_monk['fu']):.1f}, 慧={statistics.mean(smart_monk['hui']):.1f}")
