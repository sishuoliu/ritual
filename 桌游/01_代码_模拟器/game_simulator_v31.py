...
v3.0问题：僧伽97%胜率（授德点×3太强）

v3.1修正：
1. 僧伽：授德点×2（从×3），涅槃道加分从50降到35
2. 皇帝：龙气×2（从×3），天命道加分从45提高到55
3. 护法：善财道加分从50提高到60
4. 田主：舍宅道加分从55提高到65
5. 农人：勤劳道加分从50提高到60
6. 士人：清名道加分从45提高到55
7. 虔信女：菩萨道加分从45提高到55，更容易进入菩萨状态
"""
...
class Player:
    ...
    def get_final_score(self) -> float:
        ...
        if self.role == Role.EMPEROR:
            base_score = self.dragon_qi * 2 + self.prestige * 2 + self.merit * 2 + resource_bonus
        elif self.role == Role.SANGHA:
            # v3.1: 授德点×2（从×3）
            base_score = self.grant_points * 2 + self.merit * 2
        elif self.role == Role.PATRON:
            base_score = self.merit * 2 + int(self.total_donated * 1.5) + resource_bonus
        elif self.role == Role.LANDOWNER:
            base_score = self.merit * 2 + self.legacy_points * 3 + self.land_donated * 6 + resource_bonus
        elif self.role == Role.SCHOLAR:
            base_score = self.merit * 2 + self.prestige * 3 + self.literary_fame * 4 + resource_bonus
        elif self.role == Role.PEASANT:
            base_score = self.merit * 2 + self.labor_points * 3 + resource_bonus
        elif self.role == Role.UPASIKA:
            base_score = self.merit * 2 + resource_bonus
            if self.bodhisattva_mode:
                base_score += self.salvation_points * 4 + self.beings_saved * 8
        else:
            base_score = self.merit * 2 + resource_bonus
...
class GameSimulator:
    ...
    def create_game(self):
        ...
        players = [Player(role=role) for role in roles[:7]]
    ...
    def check_state_transitions(self, p, game):
        ...
        if p.role == Role.UPASIKA and not p.bodhisattva_mode:
            if p.merit >= 8 and p.merit_transfers >= 2:
                p.bodhisattva_mode = True
        
        if p.role == Role.SANGHA and game["current_round"] >= 6:
            if p.dharma_power <= 5 and p.grant_points >= 8:
                p.retreat_mode = True
    ...
    def execute_action(self, p, game, action, ap) -> int:
        if action == "none":
            ...
            targets = [t for t in game["players"] 
                      if t != p and t.role != Role.SANGHA and t.merit < 25]
            if targets:
    ...
    def decide_action(self, player, game, remaining_ap) -> str:
        ...
        if p.role == Role.EMPEROR:
            ...
            if p.coins >= 10:
                actions.append(("build_temple_patron", 0.65))
            if p.coins >= 3:
                actions.append(("donate", 0.55))
            if p.grain >= 3:
                actions.append(("trade", 0.6))
        
        elif p.role == Role.LANDOWNER:
            if p.land >= 2 and not p.temple_built and round_num >= 3:
                actions.append(("build_temple", 0.75))
            if p.land >= 1 and round_num >= 4:
                actions.append(("donate_land", 0.65))
            if p.coins >= 4 and p.land < 5 and round_num <= 5:
                actions.append(("buy_land", 0.5))
        
        elif p.role == Role.SCHOLAR:
            if p.literary_fame >= 1 and round_num >= 3:
                actions.append(("write_inscription", 0.75))
            actions.append(("compose_poem", 0.55))
            actions.append(("discuss_dharma", 0.45))
            ...
            if p.bodhisattva_mode:
                ...
                if p.merit >= 2:
                    actions.append(("transfer_merit", 0.75))
                if p.coins >= 3:
    ...
    def run_batch(self, num_games: int) -> Dict:
        ...
        return {
            ...
            "path_completions": {
                k: {"rate": v["count"]/num_games*100} for k, v in path_completions.items()
            }
        }
...
def main():
    ...
    with open("batch_simulation_results_v31.json", "w", encoding="utf-8") as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)
    
    print("\n" + "="*60)
    print("v3.1 RESULTS - Seven Roles Balanced")
    print("="*60)
    
    r = all_results[10000]
    print("\n[Win Rates - 10000 games]")
    for role, rate in sorted(r["role_win_rates"].items(), key=lambda x: x[1], reverse=True):
        bar = "#" * int(rate / 2)
        print(f"  {role}: {rate:6.2f}% {bar}")
    
    print("\n[Victory Types]")
    for vtype, rate in sorted(r["win_types"].items(), key=lambda x: x[1], reverse=True)[:10]:
