...
救赎之路 v7.5 - 差异化平衡版
设计理念：
1. 各道路有不同的最优策略（转职型 vs 稳健型）是OK的
2. 但需要确保各道路的"最终胜率"平衡
3. 转职型道路：提前胜利较难，但转职收益高
4. 稳健型道路：提前胜利更难/无，但基础得分高

调整策略：
- 布施道/渡化道/文化道：大幅提高提前胜利难度
- 土地道/勤劳道：略微降低提前胜利难度 OR 增强基础收益
- 福田道：维持现状（已经是稳健型）
"""
...
class Player:
    ...
    def __post_init__(self):
        ...
        if self.path == Path.NIRVANA:
            ...
            # 土地道：稳健型，增强基础
            self.land += 2  # 从+1增加到+2
            self.wealth += 3  # 从+2增加到+3
            ...
            self.wealth += 2  # 新增初始财富
        elif self.path == Path.SALVATION:
            # 渡化道：转职型，略微削弱
            self.merit += 4  # 从+5降到+4
            self.influence += 2
    ...
    def check_early_victory(self, players: List['Player']) -> bool:
        ...
        if self.ascension_class == AscensionClass.ARHAT:
            ...
            # 勤劳道：需要大量行动
            # 稳健型：提前胜利极难，主要靠行动效率
            return (self.kindness_count >= 15 and 
                    self.farming_count >= 15 and
                    self.merit >= 10)
    ...
    def get_ascension_bonus_multiplier(self, action: str) -> float:
        ...
        bonuses = {
            ...
            (AscensionClass.TEMPLE_MASTER, "farm"): 1.3,  # 新增：土地道转职后耕作加成
            (AscensionClass.MANJUSRI, "kindness"): 1.5,
            (AscensionClass.WANDERING_MONK, "farm"): 1.3,  # 勤劳道转职后耕作加成
            (AscensionClass.WANDERING_MONK, "kindness"): 1.3,  # 勤劳道转职后善行加成
    ...
    def check_path_completion(self) -> Tuple[bool, str, int]:
        ...
        # v7.5: 稳健型道路的路径加成更高
        if self.path in [Path.NIRVANA, Path.TEMPLE, Path.DILIGENCE]:
            base_bonus = 30
            small_bonus = 15
...
class AdvancedTester:
    ...
    def print_report(self) -> Dict:
        ...
        print(f"\n{'='*70}")
        print(f"  ADVANCED EVALUATION REPORT - {self.num_players} PLAYERS (v7.5)")
        print(f"{'='*70}")
        print(f"Total games: {self.total_games}")
        print(f"Target win rate: {target_rate:.1f}%")
        
        print(f"\n{'='*70}")
        print("  1. PATH WIN RATES")
        print(f"{'='*70}")
        ...
        print("\n  [Ascended vs Non-Ascended Win Rate]")
        for path in self.path_games:
            ...
            recommendation = "Asc-focused" if diff > 5 else ("Score-focused" if diff < -5 else "Flexible")
            print(f"    {path}: Asc {asc_rate:.1f}% vs NoAsc {no_asc_rate:.1f}% ({diff:+.1f}%) -> {recommendation}")
        
        print(f"\n{'='*70}")
        print("  3. VICTORY SOURCE")
        print(f"{'='*70}")
        ...
        print(f"\n  [Overall]")
        print(f"    Early Victory: {overall_early_rate:.1f}% ({total_early} games)")
        print(f"    Score Victory: {100-overall_early_rate:.1f}% ({total_score} games)")
        
        print(f"\n  [Early Victory by Path]")
        for path in self.path_games:
            early = self.path_early_wins.get(path, 0)
            total = self.path_wins.get(path, 0)
            early_rate = early / total * 100 if total > 0 else 0
            
            style = "Asc-type" if early_rate > 40 else ("Balanced" if early_rate > 20 else "Score-type")
            print(f"    {path}: {early_rate:.1f}% early -> {style}")
        
        print(f"\n{'='*70}")
        print("  4. COMEBACK ANALYSIS")
        print(f"{'='*70}")
        
        print("\n  [Underdog Win Rate (Rank>50% at R5, then win)]")
        for path in self.path_games:
            ...
            print(f"    {path}: comeback {comeback_rate:.1f}%, asc contribution {asc_contribution:.1f}%")
        ...
        print(f"\n  Balance: {'[OK]' if balanced else '[NEEDS WORK]'} (score: {balance_score:.2f})")
        print(f"  Early Victory Rate: {'[OK]' if early_ok else '[ADJUST]'} ({overall_early_rate:.1f}%, target 10-40%)")
        
        if balanced and early_ok:
            print("\n  [Conclusion] Game is well balanced!")
        else:
            print("\n  [Conclusion] Further adjustments needed.")
...
def main():
    print("=" * 70)
    print("  Path to Salvation v7.5 - Differentiated Balance")
    print("=" * 70)
    ...
    with open("v75_balanced_results.json", "w", encoding="utf-8") as f:
