...
救赎之路 v7.7 - 全人数平衡版
基于v7.6测试结果的最终调整：
1. 5人游戏中福田道胜率过低(5.39%)：大幅增强
2. 布施道在所有人数中偏强：进一步削弱
3. 提前胜利率偏高但各道路分化良好（这是设计目标）

核心评估指标：各道路总胜率在±5%范围内
"""
...
class Player:
    ...
    def __post_init__(self):
        ...
        if self.path == Path.NIRVANA:
            # 福田道：稳健型，大幅增强以平衡5人游戏
            self.influence += 7   # 从+6增加到+7
            self.merit += 4       # 从+3增加到+4
        elif self.path == Path.CHARITY:
            # 布施道：转职型，削弱
            self.wealth += 4      # 从+5降到+4
            ...
            self.wealth += 4      # 从+3增加到+4
            self.merit += 2       # 从+1增加到+2
    ...
    def check_early_victory(self, players: List['Player']) -> bool:
        ...
        if self.ascension_class == AscensionClass.ARHAT:
            ...
            # 布施道：转职型，提前胜利难度进一步提高
            return (self.total_donated >= 45 and 
                    self.wealth <= 2 and
                    self.merit >= 10)
...
class AdvancedTester:
    ...
    def print_report(self) -> Dict:
        ...
        print(f"\n{'='*70}")
        print(f"  BALANCE REPORT - {self.num_players} PLAYERS (v7.7)")
        print(f"{'='*70}")
        print(f"Total games: {self.total_games}")
        print(f"Target win rate: {target_rate:.1f}%")
        
        print(f"\n{'='*70}")
        print("  PATH WIN RATES (Core Balance)")
        print(f"{'='*70}")
        ...
        for path in sorted(self.path_games.keys()):
            ...
            print(f"    {path}: {style} (early: {early_rate:.1f}%)")
        ...
        print(f"  Early Victory Rate: {overall_early_rate:.1f}%")
        print(f"  Underdog Win Rate: {underdog_rate:.1f}%")
...
def main():
    print("=" * 70)
    print("  Path to Salvation v7.7 - Full Balance Test")
    print("=" * 70)
    ...
    with open("v77_final_results.json", "w", encoding="utf-8") as f:
