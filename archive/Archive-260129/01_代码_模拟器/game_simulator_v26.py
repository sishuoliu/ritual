...
v2.5问题：官员87%胜率，清官道完成率100%

v2.6核心改动：
1. 官员：
   - 清官道加分大幅降低（35→20，小成18→10）
   - 请命每次加分从5降到2
   - 初始声望降到0
   - 每次赴朝只+1声望（从+2）
...
class Player:
    ...
    def __post_init__(self):
        if self.role == Role.MERCHANT:
            ...
            self.reputation = 0  # v2.6: 初始声望降到0
            ...
            self.dharma_power = 8  # v2.6: 初始法力提高到8
    ...
    def get_final_score(self) -> float:
        ...
        if self.role == Role.FARMER:
            ...
            base_score += self.salvation_points * 3 + self.beings_saved * 8
        elif self.role == Role.MERCHANT:
            base_score += int(self.total_donated * 1.5)
        elif self.role == Role.LANDLORD:
            base_score += self.legacy_points * 3 + self.land_donated * 6
        elif self.role == Role.OFFICIAL:
            base_score += self.petitions * 2  # v2.6: 从5降到2
...
class GameSimulator:
        ...
        for p in game["players"]:
            if p.role == Role.LANDLORD:
                ...
                if not p.bodhisattva_mode:
                    p.merit += 2  # v2.6: 每轮+2功德
                else:
                    p.merit += 1  # 菩萨状态下减少自我积累
    ...
    def check_state_transitions(self, p, game):
        ...
        if p.role == Role.OFFICIAL and game["current_round"] >= 8:
    ...
    def decide_action(self, player, game, remaining_ap) -> str:
        ...
        if p.role == Role.WIDOW:
            ...
            if p.dharma_power >= 3:
                ...
                actions.append(("grant", 0.6))
        
        elif p.role == Role.MERCHANT:
            if p.charity_mode and p.coins >= 5:
                actions.append(("grand_charity", 0.75))
            if p.coins >= 4:
                actions.append(("donate_anonymous", 0.55))
            if p.coins >= 3:
                actions.append(("donate", 0.45))
            if p.grain >= 3:
            ...
            if p.reputation <= 1 and not p.retired:
                actions.append(("go_to_court", 0.6))
            if p.reputation >= 2 and not p.retired:
                actions.append(("petition", 0.55))
            if p.reputation >= 4 and round_num >= 7 and not p.retired:
...
def main():
    ...
    with open("batch_simulation_results_v26.json", "w", encoding="utf-8") as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)
    
    print("\n" + "="*70)
    print("BATCH SIMULATION RESULTS - Path to Salvation v2.6")
    print("(Six Paths - Deep Balance)")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("Results saved to batch_simulation_results_v26.json")
