...
"""
救赎之路 v7.2 - 平衡版双骰子系统
基于v7.1测试结果，调整悟道骰子以恢复平衡

问题：v7.1中福田道胜率飙升15%+（因为修行次数最多）
解决方案：
1. 降低悟道骰子收益（顿悟从+2降为+1）
2. 福田道的道路加成（修行×2）不应用于骰子额外功德
3. 心散的惩罚改为不触发（而非-1功德）
"""
...
class PracticeDiceResult(Enum):
    """悟道骰子结果 - v7.2调整版"""
    DISTRACTION = "心散"       # 1: 无额外收益（原-1，改为0）
    ORDINARY = "平凡"          # 2-3: 无额外收益
    INSIGHT = "感悟"           # 4-5: +1功德（不受道路加成）
    ENLIGHTENMENT = "顿悟"     # 6: +1功德 +1影响（原+2，降为+1）
...
class GameSimulator:
    ...
    def _roll_practice_dice(self) -> Tuple[PracticeDiceResult, int, int]:
        ...
        2-3: 平凡 - 无额外收益
        4-5: 感悟 - +1功德（不受道路加成）
        6: 顿悟 - +1功德 +1影响（原+2功德，降为+1）
        """
        roll = random.randint(1, 6)
        if roll == 1:
            return (PracticeDiceResult.DISTRACTION, 0, 0)   # 心散：无惩罚
        elif roll <= 3:
            return (PracticeDiceResult.ORDINARY, 0, 0)      # 平凡：无加成
        elif roll <= 5:
            return (PracticeDiceResult.INSIGHT, 1, 0)       # 感悟：+1功德
        else:
            return (PracticeDiceResult.ENLIGHTENMENT, 1, 1) # 顿悟：+1功德 +1影响
        ...
        if action == "donate":
            ...
            if player.influence >= 2:
                ...
                if self.use_practice_dice:
                    ...
                    # 注意：骰子额外功德不受道路加成影响！
                    dice_merit = extra_m  # 不乘以multiplier
                ...
                # 总功德 = 基础（应用道路加成） + 骰子加成（不应用道路加成）
                total_merit = base_merit + dice_merit
...
def run_test(num_players: int = 4, games: int = 10000) -> Dict:
    """运行测试"""
    print(f"\n{'='*60}")
    print(f"v7.2 平衡版测试: {num_players}人模式, {games}局")
    print(f"{'='*60}")
    
    start = time.time()
    sim = GameSimulator(num_players, use_production_dice=True, use_practice_dice=True)
    ...
    print(f"\n{'道路':<12} {'胜率':>8} {'目标':>8} {'偏差':>8} {'骰子功德':>10}")
    print("-" * 50)
    
    stats = {}
    for path in Path:
        pn = path.value
        if pn in path_games and path_games[pn] > 0:
            wins = path_wins[pn]
            total = path_games[pn]
            rate = wins / total * 100
            ...
            print(f"{pn:<12} {rate:>7.1f}% {target_rate:>7.1f}% {deviation:>+7.1f}% {avg_dice_merit:>+10.1f} {status}")
            
            stats[pn] = {
                "win_rate": round(rate, 2),
                "deviation": round(deviation, 2),
                "avg_dice_merit": round(avg_dice_merit, 2),
            }
    
    balance_score = total_deviation / num_players
    print(f"\n平衡分数: {balance_score:.2f} (越低越好，<5为合格)")
    
    # 骰子统计
    print(f"\n悟道骰子分布:")
    total_rolls = sum(sim.practice_dice_stats.values())
    for result, count in sorted(sim.practice_dice_stats.items()):
    ...
    return {
        ...
        "path_stats": stats,
        "is_balanced": balance_score < 5,
    }


if __name__ == "__main__":
    print("《救赎之路》v7.2 平衡版双骰子系统测试")
    print("调整：悟道骰子收益降低，不受道路加成")
    
    all_results = {}
    for np in [4, 5, 6]:
        result = run_test(np, 10000)
    ...
    for k, v in all_results.items():
        status = "✓ 合格" if v["is_balanced"] else "✗ 不合格"
        print(f"{k}: 平衡分数 {v['balance_score']:.2f} {status}")
    
    with open("v72_balanced_dice_results.json", "w", encoding="utf-8") as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)
    
    print("\n结果已保存到 v72_balanced_dice_results.json")
