...
"""
救赎之路 v7.9 - 布施道削弱版
核心问题：布施道在所有人数中都太强（33-38%）
解决方案：
1. 大幅提高布施道的提前胜利难度
2. 削弱布施道的转职收益
3. 增强福田道

这是最后一次调整 - 如果仍不平衡，需要重新审视整体设计
"""
...
class Player:
    ...
    def update_path_completion(self):
        if self.path == Path.NIRVANA:
            ...
            # 布施道小成条件提高
            if self.total_donated >= 9:  # 从7提高到9
                self.small_path_completed = True
        elif self.path == Path.TEMPLE:
    ...
    def check_early_victory(self, players: List['Player']) -> bool:
        ...
        if self.ascension_class == AscensionClass.ARHAT:
            ...
            # 布施道：大幅提高难度！
            return (self.total_donated >= 50 and  # 从35提高到50
                    self.wealth <= 2 and
                    self.merit >= 12)  # 从8提高到12
    ...
    def get_ascension_bonus_multiplier(self, action: str) -> float:
        ...
        bonuses = {
            ...
            (AscensionClass.MAHADANAPATI, "donate"): 1.3,  # 从1.5降到1.3
    ...
    def check_path_completion(self) -> Tuple[bool, str, int]:
        ...
        conditions = {
            ...
            Path.CHARITY: (
                self.total_donated >= 15 and self.influence >= 3,  # 从12提高到15
                self.total_donated >= 9,  # 从7提高到9
                "布施道"
            ),
            Path.TEMPLE: (
...
def main():
    ...
    with open("v79_results.json", "w", encoding="utf-8") as f:
