...
class Player:
    ...
    def check_early_victory(self, players: List['Player']) -> bool:
        ...
        if self.ascension_class == AscensionClass.ARHAT:
            ...
            # 提前胜利难度按人数调整
            donated_req = 35 + (num_players - 4) * 5
            ...
            merit_req = 12 + (num_players - 4) * 2
            return len(all_helped) >= num_players - 1 and self.merit >= merit_req and self.transfer_count >= 5
        return False
    ...
    def get_ascension_bonus_multiplier(self, action: str) -> float:
        ...
        bonuses = {
            ...
            (AscensionClass.BODHISATTVA, "transfer"): 1.5,
        }
    ...
    def check_path_completion(self) -> Tuple[bool, str, int]:
        ...
        conditions = {
            ...
            Path.CHARITY: (self.total_donated >= 12 and self.influence >= 3, self.total_donated >= 7, "布施道"),
            ...
            Path.SALVATION: (self.transfer_count >= 3 and len(self.transfer_targets | self.kindness_targets) >= 2, self.transfer_count >= 1 or len(self.kindness_targets) >= 1, "渡化道"),
        }
...
def main():
    ...
    with open("v82_results.json", "w", encoding="utf-8") as f:
