...
v2.7调整：
1. 僧侣：涅槃道加分从55降到35
2. 寡妇：菩萨道从50降到40，小成从35降到25
3. 官员：清官道加分从20提高到40，小成从10提高到25
4. 农民：勤劳道从45提高到55，小成从22提高到30
5. 地主：舍宅道从55提高到60，小成从28提高到32
6. 富商：善财道从50降到45
"""
...
class Player:
    ...
    def get_final_score(self) -> float:
        ...
        if self.role == Role.FARMER:
            base_score += self.labor_points * 2  # v2.7: 勤劳积分加倍
        elif self.role == Role.WIDOW and self.bodhisattva_mode:
            base_score += self.salvation_points * 2 + self.beings_saved * 6
        elif self.role == Role.MERCHANT:
            base_score += self.total_donated
        elif self.role == Role.LANDLORD:
            base_score += self.legacy_points * 3 + self.land_donated * 8
        elif self.role == Role.OFFICIAL:
            base_score += self.petitions * 4  # v2.7: 从2提高到4
    ...
    def decide_action(self, player, game, remaining_ap) -> str:
        ...
        if p.role == Role.WIDOW:
            ...
            if p.reputation >= 2 and not p.retired:
                actions.append(("petition", 0.65))
            if p.reputation >= 4 and round_num >= 6 and not p.retired:
                actions.append(("retire", 0.55))
            if p.coins >= 4:
...
def main():
    ...
    with open("batch_simulation_results_v27.json", "w", encoding="utf-8") as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)
    
    print("\n" + "="*70)
    print("BATCH SIMULATION RESULTS - Path to Salvation v2.7")
    print("(Six Paths - Final Balance)")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("Results saved to batch_simulation_results_v27.json")
